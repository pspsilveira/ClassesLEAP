...
if (num_iteracoes > 0)
{
  ...
  out <- epiR::epi.tests(tabela2x2_propH1, conf.level=1-alfa, digits=4)
  ...
  out <- epiR::epi.tests(tabela2x2_propH0, conf.level=1-alfa, digits=4)
  ...
  for (i in 1:num_iteracoes)
  {
    ...
    out <- epiR::epi.tests(tabela2x2_tmp, conf.level=1-alfa, digits=4)
    sumario <- summary(out)
